...
export default function ProfitabilityModule() {
  ...
  }), { spend: 0, revenue: 0, profit: 0, cogs: 0 });
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12 }}>
        {[
          { label: "הוצאה כוללת", value: `₪${totals.spend.toLocaleString()}`, color: "#7c74ff" },
          { label: "הכנסה", value: `₪${totals.revenue.toLocaleString()}`, color: "#00d4aa" },
          { label: "עלות מוצרים", value: `₪${totals.cogs.toLocaleString()}`, color: "#f59e0b" },
          { label: "רווח נקי", value: `₪${totals.profit.toLocaleString()}`, color: totals.profit > 0 ? "#10b981" : "#ef4444" },
          { label: "POAS כולל", value: `${(totals.profit / totals.spend).toFixed(2)}x`, color: "#a78bfa" },
        ].map((c) => (
      ...
      <div style={{ background: "#13132a", border: "1px solid #3a3a5a", borderRadius: 12, padding: 16, fontSize: 13, color: "#a0a0c0" }}>
        <span style={{ color: "#7c74ff", fontWeight: 600 }}>📐 נוסחת רווח: </span>
        הכנסה − הוצאת פרסום − עלות מוצר − משלוח − עמלות פלטפורמה − החזרות = <span style={{ color: "#10b981", fontWeight: 600 }}>רווח נקי</span>
      </div>
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
        {["7d", "14d", "30d", "90d"].map((d) => (
          <button
            ...
            style={{ padding: "6px 14px", borderRadius: 8, border: "none", background: dateRange === d ? "#7c74ff" : "#1a1a2e", color: dateRange === d ? "#fff" : "#8888aa", cursor: "pointer", fontSize: 12 }}
          >
            {d}
          </button>
        ))}
        <select
          ...
          style={{ marginLeft: "auto", background: "#1a1a2e", border: "1px solid #3a3a5a", color: "#e0e0ff", borderRadius: 8, padding: "8px 12px", fontSize: 13 }}
        >
          <option value="profit">מיין לפי רווח</option>
          <option value="poas">מיין לפי POAS</option>
          <option value="roas">מיין לפי ROAS</option>
          <option value="margin">מיין לפי מרג'ין</option>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          <thead>
            <tr style={{ background: "#13132a", borderBottom: "1px solid #2a2a4a" }}>
              {["קמפיין", "ציון רווח", "הוצאה", "הכנסה", "עלות מוצרים", "רווח נקי", "מרג'ין", "ROAS", "POAS", "המלצה"].map((h) => (
              ...
              <tr key={i} style={{ borderBottom: "1px solid #1e1e3a" }}>
                ...
                <td style={{ padding: "12px 16px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <div style={{ width: 40, height: 8, background: "#2a2a4a", borderRadius: 4 }}>
                      <div style={{ width: `${c.score}%`, height: "100%", background: getScoreColor(c.score), borderRadius: 4 }} />
                    </div>
                ...
                <td style={{ padding: "12px 16px", color: c.profit >= 0 ? "#10b981" : "#ef4444", fontWeight: 700 }}>
                ...
                <td style={{ padding: "12px 16px" }}>
                  <span style={{ fontSize: 11, color: c.profit >= 0 ? "#10b981" : "#ef4444" }}>
                    {c.profit < 0 ? "⛔ עצור הפסדים" : c.poas > 2 ? "🚀 הגדל תקציב" : "✅ תחזוק"}
                  </span>
                </td>
              </tr>
            ))}
