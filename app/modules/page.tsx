...
export default function ModulesPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d0d1a", color: "#e0e0ff", fontFamily: "'Segoe UI', sans-serif", direction: dir }}>
      ...
      {/* ── Header ── */}
      <div style={{ background: "linear-gradient(135deg, #1a1a2e 0%, #16213e 100%)", borderBottom: "1px solid #2a2a4a", padding: "16px 20px" }}>
        {/* Row 1: Logo + controls */}
        <div className="as-header-row">
          {/* Brand */}
          <div className="as-header-brand">
            ...
            <div style={{ minWidth: 0 }}>
              ...
              <div style={{ fontSize: 11, color: isLive ? "#00d4aa" : "#8888aa", display: "flex", alignItems: "center", gap: 6 }}>
                <span style={{ width: 6, height: 6, borderRadius: "50%", background: isLive ? "#00d4aa" : "#8888aa", display: "inline-block", flexShrink: 0 }} />
          ...
          <div className="as-header-controls">
            ...
            {/* Lang toggle */}
            <button onClick={() => setLang(lang === "he" ? "en" : "he")} style={{ padding: "6px 14px", borderRadius: 20, border: "1px solid #3a3a5a", background: "#13132a", color: "#e0e0ff", cursor: "pointer", fontSize: 12, fontWeight: 600, whiteSpace: "nowrap" }}>
              {lang === "he" ? "🇺🇸 EN" : "🇮🇱 HE"}
            </button>
          </div>
        </div>

        {/* Row 2: Live stats bar — responsive grid */}
        <div className="as-stats-grid">
          {[
            ...
            <div key={i} style={{ background: "#13132a", border: `1px solid ${m.color}33`, borderRadius: 10, padding: "10px 12px", display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontSize: 16, flexShrink: 0 }}>{m.icon}</span>
              <div style={{ minWidth: 0 }}>
                <div className="as-metric-value" style={{ fontSize: 18, fontWeight: 700, color: "#fff", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  {loading ? "..." : m.val}
                </div>
                <div className="as-metric-label" style={{ fontSize: 11, color: "#8888aa", marginTop: 1 }}>{m.label}</div>
              </div>
            </div>
          ))}
        </div>
