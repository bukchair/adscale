...
export default function IntegrationsModule({ lang }: { lang: Lang }) {
  ...
  const STATUS_CONFIG = {
    ...
    disconnected: { color: "#64748b", label: t("לא מחובר", "Disconnected"), icon: "○" },
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div className="as-integrations-summary">
        <div style={{ background: "#ffffff", border: "1px solid #10b98133", borderRadius: 12, padding: "14px 20px", display: "flex", gap: 12, alignItems: "center", flex: "1 1 auto" }}>
          <div style={{ fontSize: 26, fontWeight: 700, color: "#10b981" }}>{INTEGRATIONS.filter((i) => i.status === "connected").length}</div>
          <div style={{ fontSize: 12, color: "#64748b" }}>{t("מחוברים", "Connected")}</div>
        </div>
        <div style={{ background: "#ffffff", border: "1px solid #ef444433", borderRadius: 12, padding: "14px 20px", display: "flex", gap: 12, alignItems: "center", flex: "1 1 auto" }}>
          <div style={{ fontSize: 26, fontWeight: 700, color: "#ef4444" }}>{INTEGRATIONS.filter((i) => i.status === "error").length}</div>
          <div style={{ fontSize: 12, color: "#64748b" }}>{t("שגיאות", "Errors")}</div>
        </div>
        <div style={{ background: "#ffffff", border: "1px solid #e2e8f0", borderRadius: 12, padding: "14px 20px", display: "flex", gap: 12, alignItems: "center", flex: "1 1 auto" }}>
          <div style={{ fontSize: 26, fontWeight: 700, color: "#64748b" }}>{INTEGRATIONS.filter((i) => i.status === "disconnected").length}</div>
          <div style={{ fontSize: 12, color: "#64748b" }}>{t("לא מחוברים", "Disconnected")}</div>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(min(320px, 100%), 1fr))", gap: 16 }}>
        {INTEGRATIONS.map((intg) => {
          ...
          return (
            <div key={intg.id} style={{ background: "#ffffff", border: `1px solid ${intg.status === "connected" ? `${intg.color}33` : "#e2e8f0"}`, borderRadius: 12, padding: 20 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                  ...
                  <div>
                    ...
                    {intg.accountName && <div style={{ fontSize: 12, color: "#64748b" }}>{intg.accountName}</div>}
                  </div>
                </div>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                {intg.status === "connected" ? (
                  <>
                    <button onClick={() => triggerSync(intg.id)} disabled={syncing === intg.id} style={{ flex: 1, padding: "8px", borderRadius: 8, border: "1px solid #e2e8f0", background: "#f8fafc", color: "#1e293b", cursor: "pointer", fontSize: 12 }}>
                      {syncing === intg.id ? t("🔄 מסנכרן...", "🔄 Syncing...") : t("🔄 סנכרן עכשיו", "🔄 Sync Now")}
                    </button>
                    <button style={{ padding: "8px 12px", borderRadius: 8, border: "1px solid #e2e8f0", background: "#f8fafc", color: "#64748b", cursor: "pointer", fontSize: 12 }}>⚙️</button>
                  </>
                ) : intg.status === "error" ? (
