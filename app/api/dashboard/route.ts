...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!customerId || !devToken) throw new Error("Missing Google Ads env vars");
    ...
    const gaRes = await fetch(`https://googleads.googleapis.com/v19/customers/${customerId}/googleAds:search`, {
      method: "POST", headers, body: JSON.stringify({ query: campaignQuery }), signal: AbortSignal.timeout(8000),
    });
    if (!gaRes.ok) {
      apiErrors.push(`google:${gaRes.status}:${(await gaRes.text()).slice(0, 200)}`);
    } else {
    ...
    // Daily breakdown — use campaign level (not customer) for accurate per-campaign data
    const dailyQuery = `SELECT segments.date, metrics.cost_micros, metrics.clicks, metrics.conversions FROM campaign WHERE segments.date BETWEEN '${from}' AND '${to}'`;
    const dailyRes = await fetch(`https://googleads.googleapis.com/v19/customers/${customerId}/googleAds:search`, {
      method: "POST", headers, body: JSON.stringify({ query: dailyQuery }), signal: AbortSignal.timeout(8000),
    });
    if (dailyRes.ok) {
  ...
  } catch (e) { apiErrors.push("google:" + String(e).slice(0, 100)); }
  ...
  try {
    ...
    if (!metaToken || !metaAccountId) throw new Error("Missing Meta env vars");

    const timeRange = encodeURIComponent(JSON.stringify({ since: from, until: to }));

    // Summary
    const metaRes = await fetch(
      `https://graph.facebook.com/v19.0/act_${metaAccountId}/insights?fields=spend,clicks,impressions,actions&time_range=${timeRange}&access_token=${metaToken}`,
      { signal: AbortSignal.timeout(8000) }
    );
    if (!metaRes.ok) {
      apiErrors.push(`meta:${metaRes.status}:${(await metaRes.text()).slice(0, 200)}`);
    } else {
    ...
    const metaCampRes = await fetch(
      `https://graph.facebook.com/v19.0/act_${metaAccountId}/campaigns?fields=id,name,status,daily_budget,lifetime_budget,insights.time_range(${timeRange}){spend,clicks,impressions,actions}&access_token=${metaToken}&limit=50`,
      { signal: AbortSignal.timeout(8000) }
    );
    if (metaCampRes.ok) {
    ...
    const metaDailyRes = await fetch(
      `https://graph.facebook.com/v19.0/act_${metaAccountId}/insights?fields=spend,clicks,actions&time_increment=1&time_range=${timeRange}&access_token=${metaToken}`,
      { signal: AbortSignal.timeout(8000) }
    );
    if (metaDailyRes.ok) {
